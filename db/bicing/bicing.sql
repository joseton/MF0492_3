...
-- fin ejemplo de una carga de camion
-- ejemplo descarga de un camion
...
select @f_resto;
-- Todos los datos de quiene hayan sacado bicis de los puntos bicing
select * from salida_bici;
-- Cuántas bicicletas han salido del punto bicing 2?
select count(*) from salida_bici where id_punto = 2;
-- Nombre del usuario ha utilizado la bicicleta # 7
select U.nombre, U.apellido from usuario U join salida_bici S on S.id_salida = U.dni where id_bici = 7;
-- Todos los datos de quienes hayan ingresado bicis después de sacarlas de otros puntos
