...
-- 15. Cuántas salidas de bici se han registrado por parte de un usuario X
select count(*) from salida_bici where id_salida = "222222222";
-- 16. Nombre y apellido de los usuarios han sacado bicis del punto 2
select U.nombre, U.Apellido from usuario U join salida_bici S on S.id_salida = U.dni where id_punto = 2;
-- 17. número de veces que cada usuario ha utilizado una bici del punto Ciutat Vella
select count(*) from salida_bici where id_punto = 2 group by id_salida;
-- 18. nombre, apellido y número de veces que un usuario ha utilizado una bici del punto Ciutat Vella
select U.nombre, U.Apellido, count(*) from usuario U join salida_bici S on S.id_salida = U.dni where S.id_punto = 2 group by id_salida;
